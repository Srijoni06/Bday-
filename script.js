window.onload = function() {
  ...
  function checkCollision() {
    stars.forEach((s, i) => {
      if (s.x < wizard.x + wizard.width && s.x + 15 > wizard.x && s.y < wizard.y + wizard.height && s.y + 15 > wizard.y) {
        stars.splice(i, 1);
        score++;
      }
    });
  ...
  // Background Canvas for stars & asteroids
  ...
  // Hats floating over background
  const hatsContainer = document.createElement("div");
  hatsContainer.classList.add("hats");
  document.body.appendChild(hatsContainer);
  ...
  function createHat() {
    ...
    hat.style.fontSize=(Math.random()*1.5+1.5)+"rem"; // random size 1.5-3rem
    hat.style.animationDuration=(4+Math.random()*4)+"s"; // duration 4-8s
    hatsContainer.appendChild(hat);
    setTimeout(()=>hat.remove(), parseFloat(hat.style.animationDuration)*1000);
  }

  setInterval(createHat, 600); // spawn hat every 0.6s
};
